...
export function canDuplicateNode(editor: Editor | null): boolean {
  ...
  try {
    ...
    // Mirror duplicateNode: a text/cursor selection is duplicatable only when it
    // has an ancestor (depth >= 1) that is a real grouped node, not the doc.
    // Otherwise the button would enable but the duplicate command would no-op.
