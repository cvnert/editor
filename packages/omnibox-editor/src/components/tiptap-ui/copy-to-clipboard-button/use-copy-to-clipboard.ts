...
export async function writeToClipboard(
  ...
  try {
    if (htmlContent && navigator.clipboard && "write" in navigator.clipboard) {
      // Write both HTML and a plain-text fallback so rich targets keep the
      // formatting while plain-text targets (terminals, inputs) still receive
      // the text — browsers do not synthesize text/plain from text/html.
      ...
      // No HTML (e.g. copyWithFormatting: false) or no async clipboard API —
      // fall back to writing plain text so something is always copied.
