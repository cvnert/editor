...
function normalizeTaskMarkers(markdown: string) {
  return markdown.replace(/^(\s*[-+*]\s+)\\\[([ xX])\\\](?=\s+)/gm, "$1[$2]");
}
...
export function markdownToTiptapJson(
  ...
  const json = normalizeUrls(
    normalizeDiagramCodeBlocks(
      normalizeSparseTables(
        ensureDocument(markdownManager.parse(normalizedMarkdown)),
      ),
    ),
    options,
  );
...
function normalizeDiagramCodeBlocks(
  node: TiptapJsonContent,
): TiptapJsonContent {
  if (
    node.type === "codeBlock" &&
  ...
  return {
    ...
    ...(node.content
      ? {
          content: node.content.map((child) =>
            normalizeDiagramCodeBlocks(child),
          ),
        }
      : {}),
  };
...
function getSparseHeaderInfo(
  ...
  if (
    !hasOnlyTrailingEmptyCells ||
    isEmptyTableCell(firstCells[firstEmptyIndex - 1])
  ) {
    return null;
  }

  const secondRowGroupCells = secondCells.filter(
    (cell) => !isEmptyTableCell(cell),
  );
